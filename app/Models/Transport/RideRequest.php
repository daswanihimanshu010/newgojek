...
class RideRequest extends BaseModel
{
    ...
    public function getScheduleTimeAttribute() {
        return (isset($this->attributes['schedule_at'])) ? (\Carbon\Carbon::createFromFormat('Y-m-d H:i:s', $this->attributes['schedule_at'], 'UTC'))->setTimezone($this->attributes['timezone'])->format('Y-m-d H:i:s') : '' ;
        
    }

    public function getStartedTimeAttribute() {
        return (isset($this->attributes['started_at'])) ? (\Carbon\Carbon::createFromFormat('Y-m-d H:i:s', $this->attributes['started_at'], 'UTC'))->setTimezone($this->attributes['timezone'])->format('Y-m-d H:i:s') : '' ;
        
    }

    public function getFinishedTimeAttribute() {
        return (isset($this->attributes['finished_at'])) ? (\Carbon\Carbon::createFromFormat('Y-m-d H:i:s', $this->attributes['finished_at'], 'UTC'))->setTimezone($this->attributes['timezone'])->format('Y-m-d H:i:s') : '' ;
	...
	public function scopeRideRequestStatusCheck($query, $user_id, $check_status, $admin_service)
	{
		return $query->where('ride_requests.user_id', $user_id)
					...
					->with(['user','provider','service_type' => function($query) use($admin_service) {
                         $query->where('admin_service_id', $admin_service);
    ...
    {
        return $this->belongsTo('App\Models\Transport\RideRequestDispute','user_id','user_id');
    }
    public function lostItem()
    {
        return $this->belongsTo('App\Models\Transport\RideLostItem','user_id','user_id');
    }
