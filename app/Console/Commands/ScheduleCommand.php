...
class ScheduleCommand extends Command
{
    ...
    {
        $service = AdminService::where('admin_service_name', 'TRANSPORT')->first();  
        ...
        if(!empty($userRequest)){
            ...
            foreach($userRequest as $newRequest){
                ...
                $Providers->whereHas('service', function($query) use ($ride_delivery_id, $service) {
                ...
                if(!empty($Providers->toArray())) {
                    ...
                    $user_request = UserRequest::where('admin_service_id', $service->id)->where('request_id',$rideRequest->id)->first();
                    $user_request->delete();
