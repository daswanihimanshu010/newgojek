...
class ServiceScheduleCommand extends Command
{
    ...
    {
        $service = AdminService::where('admin_service_name', 'SERVICE')->first();  

        $userRequest = UserRequest::where('status','SCHEDULED')->where('status','!=','INPROGRESS')
                        ->where('admin_service_id', $service->id)
                        ->where('schedule_at','<=',Carbon::now()->addMinutes(2))
                        ->get();

        $hour = Carbon::now()->subHour();
        $futurehours = Carbon::now()->addMinutes(10);
        $date = Carbon::now();         
        ...
        if(!empty($userRequest)){
            ...
            foreach($userRequest as $newRequest){
                ...
                $ProviderCheck=Provider::where('is_online',1)->where('id',$serviceRequest->provider_id)->where('is_assigned',0)->where('status', 'APPROVED')
                    ->whereHas('service', function($query) use ($service_id,$admin_service_id){                     
                        $query->where('service_id',$service_id);
                        $query->where('admin_service_id',$admin_service_id);
                    })->first();
               ...
               // \Log::info($ProviderCheck);     
                ...
                if(!empty($ProviderCheck)){
                    ...
                    $Filter->admin_service_id = $admin_service_id;
                    ...
                    if(!empty($Providers->toArray())) {
                        ...
                        $user_request = UserRequest::where('admin_service_id', $admin_service_id)->where('request_id',$serviceRequest->id)->first();
