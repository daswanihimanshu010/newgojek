...
class ServeController extends Controller
{
    ...
    public function index(Request $request)
	{
		try{
			...
			$admin_service_id = AdminService::where('admin_service_name', 'SERVICE')->where('company_id', Auth::guard('provider')->user()->company_id)->first();
	...
	public function updateServe(Request $request)
	{
		...
		try{
			...
			$admin_service = AdminService::where('admin_service_name', 'SERVICE')->where('company_id', Auth::guard('provider')->user()->company_id)->first();
			...
			if($request->status == 'DROPPED') {
				...
				$getInvoice = $this->invoice($admin_service->id,$request->id,$extracharges,$extracharges_notes,$distance);
	...
	public function invoice($admin_service_id, $request_id,$extracharges,$extracharges_notes,$userdistance ='')
    {
        try {
			...
			$provider_service = ProviderService::where(['provider_id'=>$providerId,'admin_service_id'=>$admin_service_id,'service_id'=>$serviceId])->first();
			if($serviceDetails->serviceCategory->price_choose=='admin_price'){
				if(!empty($UserRequest->quantity))
					$baseFare=number_format($getbaseFare*$UserRequest->quantity,2);
				else
					$baseFare=number_format($getbaseFare,2);

				$perMiles=number_format($getperMiles,2);
				$perMins=round($getperMins,2);
			}
			else{
				if($provider_service != null){
					if(!empty($UserRequest->quantity))
						$baseFare=number_format($provider_service->base_fare*$UserRequest->quantity,2);
					else
						$baseFare=number_format($provider_service->base_fare,2);

					$perMiles=number_format($provider_service->per_miles,2);
					$perMins=number_format($provider_service->per_mins,2);
				}
			}
	...
	public function cancelServe(Request $request)
	{
		...
		$admin_service = AdminService::where('admin_service_name', 'SERVICE')->where('company_id', Auth::guard('provider')->user()->company_id)->first();
		$serviceDelete = RequestFilter::where('admin_service_id' , $admin_service->id)->where('request_id', $user_request->id)->first();
	...
	public function rate(Request $request)
    {
		...
		try {
			...
			if($ratingRequest == null) {
				...
				Rating::where('request_id', $serviceRequestid)->where('admin_service_id', $admin_service->id )->update([
								'provider_rating' => $request->rating,
								'provider_comment' => $request->comment,
						]);
			}
	...
	public function historyList(Request $request)
	{
		try{
			...
			$ServeRequests = ServiceRequest::with(array('payment'=>function($query){
								$query->select('id','service_request_id','total');
			...
			if($request->has('limit')) {
				$ServeRequests = $ServeRequests->orderBy('created_at','desc')
				->take($request->limit)->offset($request->offset)->get();
			}else{
				if($request->has('search_text') && $request->search_text != null) {
                        
			            $ServeRequests->ProviderhistorySearch($request->search_text);
			        }

			        if($request->has('order_by')) {

			            $ServeRequests->orderby($request->order_by, $request->order_direction);
			        }
				$ServeRequests = $ServeRequests->paginate(10);
			}
			$jsonResponse['total_records'] = count($ServeRequests);
			if(!empty($ServeRequests)){
				$map_icon_start = '';
				//asset('asset/img/marker-start.png');
				$map_icon_end = '';
				//asset('asset/img/marker-end.png');
				foreach ($ServeRequests as $key => $value) {
					$ServeRequests[$key]->static_map = "https://maps.googleapis.com/maps/api/staticmap?".
							"autoscale=1".
							"&size=600x300".
							"&maptype=terrian".
							"&format=png".
			...
			$jsonResponse['service'] = $ServeRequests;
	...
	public function getServiceHistorydetails(Request $request,$id)
	{
		try{
			...
			$jsonResponse['type'] = 'service';
			$ServeRequests = ServiceRequest::with(array('payment'=>function($query){
				$query->select('id','service_request_id','total','payment_mode','fixed','tax','minute','extra_charges','total','tips','payable','wallet');
			},'service'=>function($query){
				$query->select('id','service_category_id','service_name');
			...
			->select('id','booking_id','user_id','provider_id','service_id','company_id','before_image','after_image','currency','s_address','started_at','status','timezone')
			...
			if(!empty($ServeRequests)){
				...
				$map_icon_end = '';
				//asset('asset/img/marker-end.png');
					$ServeRequests->static_map = "https://maps.googleapis.com/maps/api/staticmap?".
							"autoscale=1".
							"&size=600x300".
							"&maptype=terrian".
							"&format=png".
							"&visual_refresh=true".
							"&markers=icon:".$map_icon_start."%7C".$ServeRequests->s_latitude.",".$ServeRequests->s_longitude.
							"&markers=icon:".$map_icon_end."%7C".$ServeRequests->d_latitude.",".$ServeRequests->d_longitude.
							"&path=color:0x000000|weight:3|enc:".$ServeRequests->route_key.
							"&key=".$siteConfig->server_key;
				$ServeRequests->dispute = ServiceRequestDispute::where(['provider_id'=>$providerId,'dispute_type'=>'provider','service_request_id' =>$id ])->first();
			}
			$jsonResponse['service'] = $ServeRequests;
			return Helper::getResponse(['data' => $jsonResponse]);
		}
	...
	public function saveServiceRequestDispute(Request $request,$id) {
