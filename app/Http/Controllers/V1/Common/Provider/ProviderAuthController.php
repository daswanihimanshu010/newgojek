	...
	public function document_store(Request $request) {
		...
		try {
			...
			$providerservice=Provider::with(['providerservice' => function($query){
              $query->groupBy('admin_service_id');
            ...
            foreach($providerservice[0]['providerservice'] as $k =>$v){
                if($v->admin_service_id==1){
                	$document[]='TRANSPORT';
                }else if($v->admin_service_id==2){
                	$document[]='ORDER';
                }else if($v->admin_service_id==3){
                	$document[]='SERVICE';
                }
                }
             }
            $document[]='ALL';
            $document=Document::WhereIn('type',$document)->where('status',1)->count();
