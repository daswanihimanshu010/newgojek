...
class CommonController extends Controller
{
    ...
    public function admin_services() {
        ...
        $admin_service = AdminService::where('company_id', Auth::user()->company_id)->whereNotIn('admin_service_name', ['ORDER'] )->where('status', 1)->get();
    ...
    public function chat(Request $request) 
    {
        ...
        $admin_service = AdminService::where('admin_service_name', $request->admin_service)->where('company_id', $company_id)->first();

        $user_request = UserRequest::where('request_id', $request->id)->where('admin_service_id', $admin_service->id)->where('company_id', $company_id)->first();

        if($user_request != null) {
            $chat=Chat::where('admin_service_id', $admin_service->id)->where('request_id', $request->id)->where('company_id', $company_id)->first();
            ...
            if($chat != null) {
                ...
                $chat = new Chat();
                $data[] = ['type' => $request->type, 'user' => $request->user_name, 'provider' => $request->provider_name, 'message' => $request->message  ];
                $chat->admin_service_id = $admin_service->id;
            ...
            (new SendPushNotification)->ChatPushUser($user_request->user_id, 'chat', 'New chat message received', 'New Message received', []); 
            (new SendPushNotification)->ChatPushProvider($user_request->provider_id, 'chat', 'New chat message received', 'New Message received', []); 
