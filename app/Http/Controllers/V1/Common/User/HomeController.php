...
class HomeController extends Controller
{
	...
	public function index(Request $request) {
        ...
        $menus->services = Menu::with('service')->whereHas('cities', function ($query) use($user) {
			$query->where('city_id', $user->city_id);
		})->where('company_id', $user->company_id)->where('status',1)->orderby('sort_order')->get();
        $menus->promocodes = Promocode::where('company_id', Auth::guard('user')->user()->company_id)
	...
	public function walletlist(Request $request)
	{
        if($request->has('limit')) {
            ...
            ->where('company_id',Auth::guard('user')->user()->company_id)->orderBy('id','desc');
	...
	public function countries(Request $request) {
        ...
        $country_list = CompanyCountry::with('companyCountryCities')->wherehas('companyCountryCities')->where('company_id', base64_decode($request->salt_key) )->where('status', 1)->get();
        $countries = [];
        foreach ($country_list as $country) {
            ...
            if(count($country->country->company_city) > 0) {
                foreach ($country->country->company_city as $value) {
                    if($value->city_list != null) {
    ...
    {
        $company_cities = CompanyCity::where('company_id',\Auth::guard('user')->user()->company_id)->where('country_id',\Auth::guard('user')->user()->country_id)->pluck('city_id')->all();
    ...
    public function get_chat(Request $request) {
        ...
        $admin_service = AdminService::where('admin_service_name', $request->admin_service)->where('company_id', Auth::guard('user')->user()->company_id)->first();

        $chat=Chat::where('admin_service_id', $admin_service->id)->where('request_id', $request->id)->where('company_id', Auth::guard('user')->user()->company_id)->get();
