        ...
        if($setting != null) {
            ...
            if($request->has('provider_select_timeout')) {
                ...
                if(!$request->has('destination')) {
                    $request->request->add(['destination' => '0']);
                }
            ...
            if($request->has('store_store_response_time')) {
                if(!$request->has('store_order_otp')) {
                    $request->request->add(['store_order_otp' => '0']);
                }
            ...
            foreach($request->only(['provider_select_timeout', 'provider_search_radius', 'unit_measurement', 'manual_request', 'broadcast_request', 'ride_otp', 'booking_prefix', 'destination']) as $key => $req) {
            ...
            foreach($request->only(['store_store_search_radius', 'store_store_response_time', 'store_provider_select_timeout', 'store_manual_request', 'store_broadcast_request', 'store_order_otp','store_booking_prefix']) as $key => $req) {
