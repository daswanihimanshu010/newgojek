...
IF NEW.status = 'COMPLETED' THEN
	...
	IF NEW.type = "PROVIDER" THEN
		...
		IF ( wallet > 0 AND bankdetail = 1 ) THEN
			...
			INSERT INTO `admin_wallets`(`company_id`, `admin_service_id`, `transaction_id`, `transaction_alias`, `transaction_desc`, `transaction_type`, `type`, `wallet_type`, `amount`, `open_balance`, `close_balance`) VALUES(NEW.company_id, NEW.admin_service_id, NEW.id, NEW.transaction_id, 'Amount transferred to provider', 5, 'D', 'ADMIN', NEW.wallet, admin_close_balance, (admin_close_balance - NEW.wallet));

			INSERT INTO `provider_wallets`(`company_id`, `admin_service_id`, `provider_id`, `transaction_id`, `transaction_alias`, `transaction_desc`, `type`, `amount`, `open_balance`, `close_balance`) VALUES(NEW.company_id, NEW.admin_service_id, provider, NEW.id, NEW.transaction_id, 'Amount deducted by admin', 'D', NEW.wallet, provider_close_balance, (provider_close_balance - NEW.wallet ) );
    ...
    ELSEIF NEW.type = "FLEET" THEN
		...
		SELECT `id`, `wallet_balance`, `is_bankdetail`  INTO provider, wallet, bankdetail FROM `admins` WHERE id = NEW.provider_id;
		SELECT `close_balance` INTO admin_close_balance FROM ((SELECT `close_balance` FROM `admin_wallets` ORDER BY id DESC LIMIT 1) UNION (SELECT 0) ) a LIMIT 1;
		...
		IF ( wallet > 0 AND bankdetail = 1 ) THEN
			...
			INSERT INTO `fleet_wallets`(`company_id`, `admin_service_id`, `fleet_id`, `transaction_id`, `transaction_alias`, `transaction_desc`, `type`, `amount`, `open_balance`, `close_balance`) VALUES(NEW.company_id, NEW.admin_service_id, provider, NEW.id, NEW.transaction_id, 'Amount transferred by admin', 'D', NEW.wallet, provider_close_balance, (provider_close_balance - NEW.wallet ) );
