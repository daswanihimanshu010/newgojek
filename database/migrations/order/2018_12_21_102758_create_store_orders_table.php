...
class CreateStoreOrdersTable extends Migration
{
    ...
    public function up()
    {
        Schema::connection('order')->create('store_orders', function (Blueprint $table) {
            $table->increments('id');
            $table->unsignedInteger('admin_service_id');
            ...
            $table->enum('status', ['ORDERED','RECEIVED','STORECANCELLED','CANCELLED','SEARCHING','PROCESSING','STARTED','REACHED','PICKEDUP','ARRIVED','DELIVERED','COMPLETED'])->comment('ORDERED - User creates the order, RECEIVED - Shop receives the order, STORECANCELLED - Store cancel the order ()'); 
