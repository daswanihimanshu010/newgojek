...
$router->group(['middleware' => 'auth:user'], function($app) {
	...
	$app->get('/upcoming/trips/service', 'V1\Service\User\HomeController@upcoming_trips');
	$app->get('/upcoming/trips/service/{id}', 'V1\Service\User\HomeController@getupcomingtrips');
	$app->get('/getdisputedetails', 'V1\Service\User\HomeController@getdisputedetails');
	$app->get('/dispute/service', 'V1\Service\User\HomeController@getUserdisputedetails');
	$app->get('/get_service_request_dispute/{id}', 'V1\Service\User\HomeController@get_service_request_dispute');
	$app->post('/service_request_dispute/{id}', 'V1\Service\User\HomeController@service_request_dispute');
});